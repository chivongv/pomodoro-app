...
test('Should able to change session and breaks length', async () => {
  ...
  const shortBreakLengthInput = await screen.findByLabelText(
    /short break length/i
  );
  const longBreakLengthInput = await screen.findByLabelText(
    /long break length/i
  );
  ...
  fireEvent.change(sessionLengthInput, {
    target: { value: fakeSettings.sessionLength },
  });
  fireEvent.change(shortBreakLengthInput, {
    target: { value: fakeSettings.shortBreakLength },
  });
  fireEvent.change(longBreakLengthInput, {
    target: { value: fakeSettings.longBreakLength },
  });
  fireEvent.change(sessionAmountInput, {
    target: { value: fakeSettings.sessionAmount },
  });
